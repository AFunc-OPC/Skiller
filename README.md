...
| 平台              | 安装包格式               | 说明                                                                 |
| ----------------- | ------------------------ | -------------------------------------------------------------------- |
| **macOS**   | `.dmg`                 | 支持 Intel (x64) 和 Apple Silicon (aarch64)，推荐下载 universal 版本<br />Mac用户注意需要，移除隔离属性<br/>`xattr -cr /Applications/Skiller.app` |
| **Windows** | `.msi` / `.exe`      | MSI 安装包或 NSIS 安装包，支持 x64                                   |
| **Linux**   | `.deb` / `.rpm` | DEB 适用于 Debian/Ubuntu 为通用格式                        |
...
- ✅ **仓库管理** - 在线仓库接入、拉取更新
- ✅ **ClawHub 在线发现** - 连接技能目录，在线浏览、搜索、版本查看、文件浏览、一键导入
- ✅ **Skill 分发** - 复制/软链接分发到项目
- ✅ **多环境配置** - 开发、测试、生产环境支持
- ✅ **代理设置** - 系统代理/自定义代理，支持身份验证和跳过规则
- ✅ **国际化** - 中英文双语支持
- ✅ **明暗主题** - 主题切换
